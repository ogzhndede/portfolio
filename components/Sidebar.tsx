...
const SOCIAL_LINKS = [
  {
    href: "https://www.linkedin.com/in/oguzhandede/",
    label: "LinkedIn",
    icon: (
  ...
  {
    href: "mailto:oguzhandede99@gmail.com",
    label: "Email",
    icon: (
  ...
  {
    href: "https://github.com/ogzhndede",
    label: "GitHub",
    icon: (
      <svg className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
...
export default function Sidebar({
      ...
      {NAV.map((item) => {
        ...
        return (
          <Link
            ...
            onClick={onClick}
            className={`
              block w-full rounded-2xl border px-6 py-4 text-left text-sm font-bold uppercase tracking-widest transition-all duration-300
              ${active
                ? "border-[#8b8aef]/50 bg-[#8b8aef]/10 text-white shadow-[0_0_20px_rgba(139,138,239,0.1)] translate-x-1"
                : "border-white/5 bg-transparent text-white/40 hover:bg-white/5 hover:text-white/80 hover:border-white/10"}
            `}
          >
            {item.label}
          </Link>
    ...
    <div className="flex items-center justify-center gap-4 py-6">
      {SOCIAL_LINKS.map((social) => (
        <a
          ...
          className="group relative rounded-xl border border-white/5 bg-white/5 p-3 text-white/40 transition-all duration-300 hover:border-[#8b8aef]/40 hover:bg-[#8b8aef]/10 hover:text-[#8b8aef] hover:-translate-y-1"
  ...
  return (
    <>
      {/* Mobile Top Bar */}
      <div className="sticky top-0 z-40 flex items-center justify-between border-b border-white/10 bg-[#0a0a0a]/90 font-mono px-6 py-4 backdrop-blur-xl md:hidden">
        <div className="flex items-center gap-3">
          <div className="relative h-10 w-10 overflow-hidden rounded-full border border-white/20">
            <Image src={imageSrc} alt="Profile" fill className="object-cover" />
          </div>
          <div className="text-sm font-bold tracking-tighter uppercase text-white">{title}</div>
        </div>

        <button
          onClick={() => setOpen(true)}
          className="rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-xs font-bold uppercase tracking-widest text-white/80 hover:bg-white/10 transition-all"
        >
          Menu
        </button>
      </div>
      ...
      {open && (
        <div className="fixed inset-0 z-50 md:hidden font-mono">
          ...
          />
          <div className="absolute left-0 top-0 flex h-full w-[80vw] flex-col border-r border-white/10 bg-[#0a0a0a] p-8 animate-in slide-in-from-left duration-300">
